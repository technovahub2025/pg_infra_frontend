...
import { useDeleteTask, useKanbanColumns, useSaveKanbanColumns, useUpdateTask } from '../../hooks/useTasks';
  ...
  const handleTaskSave = useCallback(
    async (values) => {
      ...
      const payload = {
        ...
        project: values.project || editingTask.projectId || editingTask.project?.id || editingTask.project,
        ...
        backupReviewer: values.backupReviewer || undefined,
  ...
  const handleDeleteColumn = useCallback(
    async (columnId) => {
      ...
      try {
        await Promise.all(
          affected.map((task) =>
            taskService.update(task.id, { status: fallbackColumnId, project: task.projectId || task.project?.id || task.project }),
          ),
        );
    ...
    [columns, localTasks, queryClient, saveKanbanColumns],
  );
